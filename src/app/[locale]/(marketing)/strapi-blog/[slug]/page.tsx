...
export async function generateMetadata({
  ...
  // Check if we have a valid post with attributes
  ...
  return {
    title: `${post.attributes.title} | ${t('title')} (Strapi)`,
...
export default async function StrapiBlogPostPage({
  ...
  try {
    ...
    console.log('Strapi post response:', JSON.stringify(response, null, 2));
  } catch (err) {
    ...
    return (
      <div className="container py-8">
        <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">
          {error || 'Post not found or invalid data'}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container py-8">
      <div className="mb-6">
        <a href={`/${locale}/strapi-blog`} className="text-primary hover:underline">
          ← Back to Blog
        </a>
      </div>
